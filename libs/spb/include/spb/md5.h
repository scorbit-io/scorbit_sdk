...
/* forward declaration */
inline void Transform(uint32_t* buf, uint32_t* in);

inline unsigned char PADDING[64] = {
 ...
 */
inline void MD5Update(MD5_CTX* mdContext, unsigned char* inBuf, unsigned int inLen)
{
    uint32_t in[16];
    int mdi;
    unsigned int i, ii;
 ...
 */
inline void MD5Final(unsigned char hash[], MD5_CTX* mdContext)
{
    uint32_t in[16];
    int mdi;
    unsigned int i, ii;
